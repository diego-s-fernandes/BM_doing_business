...
teste <- bbo_ind %>% 
  ...
  t <- ggloop::ggloop(teste,aes_loop(x= db_year,y = c(abertura_de_empresa,comercio_entre_fronteiras), fill = grupo_eco))%L+%
...
empresa <- t[["fill.grupo_eco"]][["x.db_year_y.abertura_de_empresa"]]
 empresa <- empresa +
   geom_text(aes(label = abertura_de_empresa), position=position_dodge(width=0.9), vjust=-0.25, size = 4, color = "black")
