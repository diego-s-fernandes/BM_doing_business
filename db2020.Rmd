...
bbo <- bind_rows(bbo, bra)  
  ...
  bbo <- bbo %>% 
    ...
    summarise(m_pontuacao = round(mean(pontuacao),2))
...
listapaises <- countrycode::codelist_panel
...
bbo_ind_spread <- bbo_ind %>% 
  ...
  ind_loop <- ggloop::ggloop(bbo_ind_spread,aes_loop(x= db_year,y = c(abertura_de_empresa:resolucao_de_insolvencia), fill = grupo_eco))%L+%
          ...
          ylim(0,100)%L+%
    scale_fill_manual(values=c("#CC6666", "#9999CC", "#66CC99"))%L+%
...
contratos <- contratos + geom_text(aes(label = execucao_de_contratos), position=position_dodge(width=0.9), vjust=-0.25, size = 3, color = "black") 
