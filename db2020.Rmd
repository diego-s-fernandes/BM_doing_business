...
ggsave(filename = "ggbrics.pdf", width = 40, height = 20, units = "cm")
ggsave
...
bbo_ind_list <- unique(bbo_ind$indicador) # criando a lista de nome dos indicadores

for (i in seq_along(bbo_ind_list)) {
   print(ggplot(subset(bbo_ind,bbo_ind$indicador == bbo_ind_list[i]),
                 aes(db_year,pontuacao, fill = grupo_eco, label = pontuacao ))+
       geom_bar(stat="identity", position=position_dodge())   +
        geom_text(aes(label= pontuacao), position = "dodge",hjust=0.5,vjust= -1, size = 6, color = "black")+
     
      ylim(0,100)+
       ggtitle (paste("Pontuação em:",bbo_ind_list[i] ))+
      labs(
        x = "Ano",
        y = "Pontuação",
