...
diego_theme <- theme(text = element_text(color = "gray20", size = 12),
...
bbo_ind <- bbo_ind %>% 
    group_by(economy, indicador,grupo_eco, db_year) %>% 
    summarise(m_pontuacao = round(mean(pontuacao),2))
...
bra_ind$indicador[bra_ind$indicador == "score_starting_a_business" ] <- "Abertura de Empresa" 
bra_ind$indicador[bra_ind$indicador == "score_getting_electricity" ] <- "Obtenção de Eletricidade"
bra_ind$indicador[bra_ind$indicador == "score_registering_property" ] <- "Registro de Propriedades"
bra_ind$indicador[bra_ind$indicador == "score_getting_credit" ] <- "Obtenção de Crédito"
bra_ind$indicador[bra_ind$indicador == "score_protecting_minority" ] <- "Proteção dos Investidores Minoritários"
bra_ind$indicador[bra_ind$indicador == "score_trading_across_borders" ] <- "Comércio entre Fronteiras"
bra_ind$indicador[bra_ind$indicador == "score_resolving_insolvency" ] <- "Resolução de Insolvência"
bra_ind$indicador[bra_ind$indicador == "score_dealing_with_construction" ] <- "Obtenção de alvarás de construção"
bra_ind$indicador[bra_ind$indicador == "score_enforcing_contracts" ] <- "Execução de Contratos"
bra_ind$indicador[bra_ind$indicador == "score_paying_taxes" ] <- "Pagamento de Impostos"

bra_ind_list <- unique(bra_ind$indicador)
f_ind_bra <- function(...){
for (i in seq_along(bra_ind_list))
   print(ggplot(subset(bra_ind,bra_ind$indicador == bra_ind_list[i]),
                 aes(db_year,pontuacao,fill= "#00AFBB" ))+
    geom_bar(stat="identity", position=position_dodge(), fill= "#00AFBB")+
      ylim(0,100)+
      geom_text(aes(label= pontuacao), position = "dodge",hjust=0.5,vjust= -1, size = 4, color = "black")+
